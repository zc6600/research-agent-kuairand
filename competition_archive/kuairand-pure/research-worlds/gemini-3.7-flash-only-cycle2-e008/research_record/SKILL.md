...
A fresh Scientist is cognitively fresh, not scientifically blank. You should drop the previous Scientist's hidden reasoning trajectory while using task facts, the current implementation, and selected operational memory.
...
6. `research_record/EXPLORE.md`, `research_record/OPTIMIZE.md`, `research_record/ENGINEERING.md`, and `research_record/KNOWLEDGE.md`.
7. `research_record/RESEARCH_INTUITION.md` and `research_record/DO_BETTER.md` as accumulated scientific and research-process progress.
8. The active Serial or Parallel coordination input.

Do **not** read `research_record/RESEARCH_BRIEF.md`. It is META-owned trajectory compression and is intentionally excluded from Scientist input to reduce anchoring on prior results.
