...
rows = []
log_f2 = 'log_standard_4_22_to_5_08_pure.csv' if os.path.exists(f'{D}/log_standard_4_22_to_5_08_pure.csv') else 'log_public_4_22_to_4_28_pure.csv'
for f in ('log_standard_4_08_to_4_21_pure.csv', log_f2):
...
for mode,desc in [('base','5 域（当前 kit）'),('item','+4 物品侧 = 9 域'),('cwm13','CWM 全 13 域')]:
    ...
    for seed in range(3):
        ...
        for ep in range(40):
            ...
            if p>best+1e-5: best=p; bad=0; state=(m.V.copy(),m.W.copy(),np.float32(m.b))
                ...
                bad+=1
                if bad>=4: break
        target_u, target_y, target_X = (ute, yte, Xte) if len(yte) > 0 else (uva, yva, Xva)
        split_name = "test" if len(yte) > 0 else "valid"
        scores.append(evaluate(target_u, target_y, m.predict(target_X)))
    ...
    print(f"{desc:20s} ({nf:2d}域) | {split_name} GAUC {g:.4f} | nDCG@5 {n5:.4f} | primary {pr:.4f} ± {sd:.4f}")
