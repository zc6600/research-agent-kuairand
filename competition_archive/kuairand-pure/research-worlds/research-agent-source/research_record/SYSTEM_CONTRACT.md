...
This file is the canonical source for **authority, ownership, data access, coordination boundaries, and State/version semantics**. It does not prescribe a scientific workflow.
...
- `RESEARCH_BRIEF.md` is META's concise descriptive compression for the next fresh Scientist.
...
Scientist reads the full externalized progress available in the research world, including the current `STATE.yaml`, `RESEARCH_INTUITION.md`, and `DO_BETTER.md`. These files are accumulated/descriptive progress, not authority: Scientist may challenge, reinterpret, weaken, or reject their scientific interpretations when actual evidence warrants it.
...
The scientific handoff is the persistent research environment, including `RESEARCH_BRIEF.md`, the current `STATE.yaml` and implementation, underlying evidence and records, scientific intuition, process lessons, and Scientist reports. These are descriptive or fallible memory rather than coordination control.
...
RESEARCH_BRIEF.md
  META-maintained concise, human-readable, lossy compression for a fresh Scientist
...
A fresh Scientist should not read every historical report by default. `RESEARCH_BRIEF.md` and the maintained memory stack are the normal cold-start handoff. Historical reports are deeper provenance to inspect when a summary is insufficient, ambiguous, worth challenging, or linked from the current State.
