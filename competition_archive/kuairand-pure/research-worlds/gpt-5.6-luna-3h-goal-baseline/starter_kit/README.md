...
`--model` 可选 `fm`（默认优化后的 FM ranker）/ `pop`（trivial baseline）/ `random`（下界，用于自检评测代码）。
优化 ranker 使用 `user_id、video_id、author_id、tab、dur_bucket、tab×hour` 六个无泄漏字段，并对正例加权 2.0；
`--official-control` 可复现原始五域、未加权 FM。FM 全程约 40 秒（CPU，单核）。
...
本地 public-validation 上，优化后的六域 ranker（seed=0）达到 GAUC 0.6718 / nDCG@5 0.5374 / primary 0.6046；
该结果使用 `starter_kit/evaluate.py`，不是 hidden-test 分数。
...
| `baseline.py` | 官方控制、优化 FM ranker、popularity 与 random baseline。 |
| `baseline_scores.json` | 官方发布的分数 + 种子方差 + 收敛参数。 |
| `submit.py` | 生成 / 校验提交文件（默认使用优化 ranker；`--official-control` 可生成原始 FM）。 |
