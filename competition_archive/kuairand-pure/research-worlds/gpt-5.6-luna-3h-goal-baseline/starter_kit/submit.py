...
用法：
    python3 submit.py --make   submission.csv     # 用优化 FM ranker 生成提交（--official-control 可切回原始 FM）
    python3 submit.py --check  submission.csv     # 校验格式与对齐
    python3 submit.py --score  submission.csv     # 校验并打分（仅本地 valid 可用）
"""
import argparse, csv, sys
try:
    from .data import load, encode, FIELDS, RANKING_FIELDS
...
if __name__ == '__main__':
    ...
    ap.add_argument('--official-control', action='store_true',
                    help='make the original 5-field unweighted FM control')
    ...
    if a.make:
        ...
        fields = FIELDS if a.official_control else RANKING_FIELDS
        ...
        weights = np.where(ytr > 0.5, positive_weight, 1.0).astype(np.float32)
        ...
        for ep in range(40):
            ...
            for i in range(0, len(idx), 8192):
                sl = idx[i:i+8192]
                m.step(Xtr[sl], ytr[sl], weights[sl])
        ...
        label = 'official FM control' if a.official_control else 'optimized FM ranker'
        print(f"已写出 {a.path}：{len(rows):,d} 行（split={a.split}，{label}）")
    else:
