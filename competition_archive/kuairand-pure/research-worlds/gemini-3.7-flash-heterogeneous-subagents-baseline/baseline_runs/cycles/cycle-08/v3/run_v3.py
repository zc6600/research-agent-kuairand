"""Round 08 - Variant v3: Causal Last-Interacted Scenario / Tab Transition (11 Fields DCN-FM)
11 Fields:
- user_id
- video_id
- author_id
- tab (current recommendation scenario)
...
- register_days_range
- last_tab (causally tracked immediately preceding interaction scenario/tab; 'UNK' if first interaction)

Model: Explicit Cross Layer + Factorization Machine (DCN-FM)
Cross layer formulation: x_1 = x_0 * (x_0 @ W_c + b_c) + x_0, where x_0 = E_concat in R^176 (11 fields * 16 dim)
...
def load_data_with_causal_last_tab(data_dir):
    print(f"Loading data from {data_dir} and computing causal last_tab...")
    t0 = time.time()
    ...
    # 3. Load interaction logs from train and public valid
    raw_train = []
    with open(os.path.join(data_dir, 'log_standard_4_08_to_4_21_pure.csv')) as fh:
        for idx, r in enumerate(csv.DictReader(fh)):
            ...
            raw_train.append((time_ms, 'train', idx, date, uid, vid, author, tab, dur, label, *user_feats))
    ...
    with open(os.path.join(data_dir, 'log_public_4_22_to_4_28_pure.csv')) as fh:
        for idx, r in enumerate(csv.DictReader(fh)):
            ...
            time_ms = int(r['time_ms'])
            user_feats = u_ext.get(uid, ['UNK'] * len(USER_FE))
            raw_valid.append((time_ms, 'valid', idx, date, uid, vid, author, tab, dur, label, *user_feats))

    print(f"Loaded raw interactions: train={len(raw_train)}, valid={len(raw_valid)}")

    # 4. Group all interactions chronologically per user to compute strictly causal last_tab
    user_events = defaultdict(list)
    for ev in raw_train:
        user_events[ev[4]].append(ev)
    for ev in raw_valid:
        user_events[ev[4]].append(ev)

    train_last_tab = [None] * len(raw_train)
    valid_last_tab = [None] * len(raw_valid)

    for uid, events in user_events.items():
        events.sort(key=lambda x: x[0])  # sort strictly by time_ms
        prev_tab = 'UNK'
        for ev in events:
            time_ms, split, orig_idx, date, uid, vid, author, tab = ev[:8]
            if split == 'train':
                train_last_tab[orig_idx] = prev_tab
            else:
                valid_last_tab[orig_idx] = prev_tab
            prev_tab = tab  # update state to current interaction's tab

    # Assemble structured split rows: (date, uid, vid, author, tab, dur, label, *user_feats, last_tab)
    splits = {'train': [], 'valid': []}
    for idx, ev in enumerate(raw_train):
        date, uid, vid, author, tab, dur, label = ev[3:10]
        user_feats = ev[10:]
        lt = train_last_tab[idx]
        splits['train'].append((date, uid, vid, author, tab, dur, label, *user_feats, lt))

    for idx, ev in enumerate(raw_valid):
        date, uid, vid, author, tab, dur, label = ev[3:10]
        user_feats = ev[10:]
        lt = valid_last_tab[idx]
        splits['valid'].append((date, uid, vid, author, tab, dur, label, *user_feats, lt))

    print(f"Computed causal last_tab in {time.time() - t0:.2f}s")
    print(f"  Train UNK last_tab count: {train_last_tab.count('UNK')} / {len(train_last_tab)}")
    print(f"  Valid UNK last_tab count: {valid_last_tab.count('UNK')} / {len(valid_last_tab)}")
    return splits

def encode_features(splits):
    print("Encoding features across 11 fields (including 20 Log Duration Bins & Causal Last Tab)...")
    t0 = time.time()
    tr = splits['train']
    ...
    def raw_features(x):
        # x is (date, uid, vid, author, tab, dur, label, user_active_degree, follow, fans, friend, reg_days, last_tab)
        ...
        return [
            ...
            x[3],                # 2: author_id
            x[4],                # 3: tab
            dur_bucket,          # 4: dur_bucket
            x[7],                # 5: user_active_degree
            x[8],                # 6: follow_user_num_range
            x[9],                # 7: fans_user_num_range
            x[10],               # 8: friend_user_num_range
            x[11],               # 9: register_days_range
            x[12],               # 10: last_tab
        ]
...
def run_experiment(k=16, lr=0.001, l2=1e-6, epochs=20, bs=8192, patience=4, seed=0):
    ...
    splits = load_data_with_causal_last_tab(data_dir)
    ...
    history = []
    print("\nStarting DCN_FM training with Causal Last Tab (11 Fields)...")
    ...
    results = {
        'variant': 'v3',
        'hypothesis': 'Causal Last-Interacted Scenario / Tab Transition (11-Field DCN-FM)',
        'fields': FIELDS,
        'hyperparameters': {
            'k': k,
            'lr': lr,
