"""Round 08 - Variant v1: Causal Last-Interacted Author Feature (11-Field DCN-FM)
11 Fields:
- user_id
- video_id
- author_id
...
- register_days_range
- last_author_id (Causal last-interacted author_id in chronological order per user, UNK for first interaction)

Model: Explicit Cross Layer + Factorization Machine (DCN-FM)
Cross layer formulation: x_1 = x_0 * (x_0 @ W_c + b_c) + x_0, where x_0 = E_concat in R^176 (11 fields * k=16)
...
class DCN_FM:
    def __init__(self, dim, num_fields=11, k=16, lr=0.001, l2=1e-6, seed=0):
        ...
        self.k = k
        self.D = num_fields * k  # 176 for 11 fields
        self.lr = lr
        self.l2 = l2
...
def load_data(data_dir):
    ...
    # 3. Load interaction logs for train and valid
    train_raw = []
    ...
    print(f"Read {len(train_raw)} train rows and {len(valid_raw)} valid rows.")
    
    # Combine and sort chronologically by time_ms
    # Tie-breaker: preserve original file index within split (train before valid if identical time)
    combined = sorted(train_raw + valid_raw, key=lambda x: (x["time_ms"], 0 if x["split"] == "train" else 1, x["orig_idx"]))
    
    # Maintain causal running state of author_id per user
    user_last_author = {}
    train_processed = [None] * len(train_raw)
    valid_processed = [None] * len(valid_raw)
    
    for item in combined:
        uid = item["uid"]
        cur_author = item["author"]
        last_author = user_last_author.get(uid, "UNK")
        user_last_author[uid] = cur_author  # Update state for future interactions
        
        # Structure: (date, uid, vid, author, tab, dur, label, *user_feats, last_author)
        row_tuple = (
            item["date"],
            item["uid"],
            item["vid"],
        ...
        if item["split"] == "train":
            train_processed[item["orig_idx"]] = row_tuple
        else:
            valid_processed[item["orig_idx"]] = row_tuple
    ...
    unk_train = sum(1 for r in train_processed if r[-1] == "UNK")
    unk_valid = sum(1 for r in valid_processed if r[-1] == "UNK")
    print(f"Causal last_author assigned. UNK in train: {unk_train} ({unk_train/len(train_processed):.2%}), UNK in valid: {unk_valid} ({unk_valid/len(valid_processed):.2%})")
    print(f"Loaded splits: {{k: len(v) for k, v in splits.items()}} in {time.time() - t0:.2f}s")
    return splits

def encode_features(splits):
    print("Encoding features across 11 fields (including 20 Logarithmic Duration Bins and Causal Last-Interacted Author)...")
    t0 = time.time()
    tr = splits["train"]
    ...
    def raw_features(x):
        # x is (date, uid, vid, author, tab, dur, label, user_active_degree, follow, fans, friend, reg_days, last_author)
    ...
    # Build vocabularies strictly from train split
...
def run_experiment(k=16, lr=0.001, l2=1e-6, epochs=20, bs=8192, patience=4, seed=0):
    ...
    results = {
        "variant": "v1",
        "hypothesis": "Causal Last-Interacted Author Feature (11 fields in Log-Duration DCN-FM)",
        "fields": FIELDS,
        "hyperparameters": {
            "k": k,
            "lr": lr,
