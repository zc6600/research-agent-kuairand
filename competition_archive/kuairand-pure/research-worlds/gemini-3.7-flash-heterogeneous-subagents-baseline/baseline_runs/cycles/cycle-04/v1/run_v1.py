"""Round 04 - Variant v1: Fine-Grained 30-Quantile Duration Bucketing in DCN-FM
10 Fields:
- user_id
- video_id
- author_id
- tab
- dur_bucket (30 quantiles fitted strictly on training partition)
- user_active_degree
...
def encode_features(splits):
    print("Encoding features across 10 fields (with 30-quantile duration buckets)...")
    t0 = time.time()
    tr = splits['train']
    # Calculate duration bucket edges on train split only (30 quantiles -> 29 edges)
    edges = np.quantile(np.asarray([x[5] for x in tr]), np.linspace(0, 1, 31)[1:-1])
...
def run_experiment(k=16, lr=0.001, l2=1e-6, epochs=20, bs=8192, patience=4, seed=0):
    ...
    history = []
    print("\nStarting DCN_FM training with 30-quantile duration buckets...")
    ...
    results = {
        'variant': 'v1',
        'hypothesis': 'Fine-Grained 30-Quantile Duration Bucketing in DCN-FM',
        'fields': FIELDS,
        'hyperparameters': {
            'k': k,
            'lr': lr,
