"""Round 02 - Variant v1: Compact Rank Factorization (k=8, L2=1e-6) on 10-field representation
Fields:
- user_id
- video_id
- author_id
...
Hyperparameters: k=8, lr=0.001, l2=1e-6, batch_size=8192, max_epochs=25, patience=4
...
class FM:
    def __init__(self, dim, k=8, lr=0.001, l2=1e-6, seed=0):
...
def encode_features(splits):
    ...
    t0 = time.time()
    tr = splits['train']
    # Calculate duration bucket edges on train split only (strictly preventing data leakage)
...
def run_experiment(k=8, lr=0.001, l2=1e-6, epochs=25, bs=8192, patience=4, seed=0):
    ...
    results = {
        'variant': 'v1',
        'hypothesis': 'Compact Rank Factorization (k=8, L2=1e-6)',
        'fields': FIELDS,
        'hyperparameters': {
            'k': k,
            'lr': lr,
