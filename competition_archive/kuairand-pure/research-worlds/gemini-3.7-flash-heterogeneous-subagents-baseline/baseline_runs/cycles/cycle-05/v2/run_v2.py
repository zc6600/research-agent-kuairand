"""Round 05 - Variant v2: Day-of-Week + Hour-of-Day Dual Context (12 Fields in Log-Duration DCN-FM)
12 Fields:
1.  user_id
2.  video_id
3.  author_id
...
11. dow (day-of-week 0-6 extracted from date)
12. hour (hour-of-day 0-23 extracted from hourmin)

Model: Explicit Cross Layer + Factorization Machine (DCN-FM)
Cross layer formulation: x_1 = x_0 * (x_0 @ W_c + b_c) + x_0, where x_0 = E_concat in R^192 (12 * 16)
...
class DCN_FM:
    def __init__(self, dim, num_fields=12, k=16, lr=0.001, l2=1e-6, seed=0):
        rng = np.random.default_rng(seed)
        self.dim = dim
        self.num_fields = num_fields
        self.k = k
        self.D = num_fields * k  # 12 * 16 = 192
        self.lr = lr
        self.l2 = l2
...
def get_dow(date_int):
    # e.g. 20220411 -> Monday is 0, Sunday is 6
    d_str = str(date_int)
    y, m, d = int(d_str[:4]), int(d_str[4:6]), int(d_str[6:8])
    return datetime.date(y, m, d).weekday()

def get_hour(hourmin_str):
    # e.g. "1900" -> 19
    return int(hourmin_str) // 100
    ...
    for f in log_files:
        ...
        with open(f_path) as fh:
            for r in csv.DictReader(fh):
                ...
                date = int(r['date'])
                if date not in date2dow:
                    date2dow[date] = str(get_dow(date))
                dow = date2dow[date]
                hour = str(get_hour(r['hourmin']))
                user_feats = u_ext.get(uid, ['UNK'] * len(USER_FE))
                rows.append((date, uid, vid, author, tab, dur, label, dow, hour, *user_feats))
...
def encode_features(splits):
    print("Encoding features across 12 fields with 20 Logarithmic Duration Bins and DOW + Hour Context...")
    t0 = time.time()
    tr = splits['train']
    ...
    def raw_features(x):
        # x is (date, uid, vid, author, tab, dur, label, dow, hour, user_active_degree, follow, fans, friend, reg_days)
...
def run_experiment(k=16, lr=0.001, l2=1e-6, epochs=20, bs=8192, patience=4, seed=0):
    ...
    results = {
        'variant': 'v2',
        'hypothesis': 'Day-of-Week + Hour-of-Day Dual Context (12 fields in Log-Duration DCN-FM)',
        'fields': FIELDS,
        'hyperparameters': {
            'k': k,
            'lr': lr,
