"""Round 05 - Variant v1: Hour-of-Day Contextual Conditioning (11 Fields in DCN-FM)
11 Fields:
- user_id
- video_id
- author_id
...
- hour (0-23 extracted from hourmin)
- user_active_degree
    ...
    for f in log_files:
        ...
        with open(f_path) as fh:
            for r in csv.DictReader(fh):
                ...
                rows.append((date, uid, vid, author, tab, dur, hour, label, *user_feats))
...
def encode_features(splits):
    print("Encoding features across 11 fields with 20 Log Duration Bins & Hour of Day...")
    t0 = time.time()
    tr = splits["train"]
    ...
    def raw_features(x):
        # x is (date, uid, vid, author, tab, dur, hour, label, user_active_degree, follow, fans, friend, reg_days)
...
def run_experiment(k=16, lr=0.001, l2=1e-6, epochs=20, bs=8192, patience=4, seed=0):
    ...
    history = []
    print("\nStarting DCN_FM training with 11 Fields (Hour-of-Day Context + 20 Log Duration Bins)...")
    ...
    results = {
        "variant": "v1",
        "hypothesis": "Hour-of-Day Contextual Conditioning (11 Fields in DCN-FM)",
        "fields": FIELDS,
        "hyperparameters": {
            "k": k,
            "lr": lr,
