"""Round 05 - Variant v3: Coarse Daypart Context in Log-Duration DCN-FM
11 Fields:
- user_id
- video_id
- author_id
...
- register_days_range
- daypart (4 coarse dayparts based on hourmin: Night[0-5]->0, Morning[6-11]->1, Afternoon[12-17]->2, Evening[18-23]->3)

Model: Explicit Cross Layer + Factorization Machine (DCN-FM)
Cross layer formulation: x_1 = x_0 * (x_0 @ W_c + b_c) + x_0, where x_0 = E_concat in R^176 (11 fields * 16)
...
def get_daypart(hourmin_str):
    try:
        hour = int(hourmin_str) // 100
        # 0: Night (00:00 - 05:59)
    ...
    for f in log_files:
        ...
        with open(f_path) as fh:
            for r in csv.DictReader(fh):
                ...
                date = int(r['date'])
                daypart = get_daypart(r.get('hourmin', ''))
                user_feats = u_ext.get(uid, ['UNK'] * len(USER_FE))
                rows.append((date, uid, vid, author, tab, dur, label, *user_feats, daypart))
...
def encode_features(splits):
    print("Encoding features across 11 fields with 20 Logarithmic Duration Bins & 4 Coarse Dayparts...")
    t0 = time.time()
    tr = splits['train']
    ...
    def raw_features(x):
        # x is (date, uid, vid, author, tab, dur, label, user_active_degree, follow, fans, friend, reg_days, daypart)
...
def run_experiment(k=16, lr=0.001, l2=1e-6, epochs=20, bs=8192, patience=4, seed=0):
    ...
    results = {
        'variant': 'v3',
        'hypothesis': 'Coarse Daypart Context (11 fields: Morning, Afternoon, Evening, Night in Log-Duration DCN-FM)',
        'fields': FIELDS,
        'hyperparameters': {
            'k': k,
            'lr': lr,
