"""Round 07 - Variant v3: User Historical Dwell / Long-View Tendency Prior Field (11 fields in DCN-FM)

Hypothesis:
Adding a Bayesian smoothed empirical user historical long-view completion rate p_u = (pos_u + 10 * p_bar) / (imp_u + 10)
computed strictly from training split and discretized into 20 uniform bins on [0, 1] as an 11th categorical field
(user_hist_rate_bin) will provide the model with an explicit global prior on user dwell tendency, improving
generalization on personalized long-view ranking.
...
- register_days_range
- user_hist_rate_bin (20 uniform bins on [0, 1] of empirical user historical long_view completion rate)

Model: DCN-FM (Explicit Cross Layer + Factorization Machine, D = 11 * 16 = 176)
...
def encode_features(splits):
    print("Encoding features across 11 fields (including 20 Log Duration Bins & 20 User Historical Rate Bins)...")
    t0 = time.time()
    tr = splits['train']
    
    # 1. Calculate log-transformed duration bin edges strictly on train split
    ...
    print(f"Created {len(dur_edges)} internal cutoffs for 20 log duration bins:")
    print("  Cutoffs:", np.round(dur_edges, 4).tolist())

    # 2. Compute empirical user historical long_view completion rate strictly from train split
    # p_u = (pos_u + 10 * p_bar) / (imp_u + 10)
    user_pos = {}
    user_imp = {}
    total_pos = 0
    total_imp = 0
    for x in tr:
        uid = x[1]
        lbl = x[6]
        user_imp[uid] = user_imp.get(uid, 0) + 1
        if lbl == 1:
            user_pos[uid] = user_pos.get(uid, 0) + 1
        total_imp += 1
        total_pos += lbl

    p_bar = float(total_pos) / float(total_imp)
    print(f"Global train long_view positive rate p_bar = {p_bar:.6f} ({total_pos}/{total_imp})")
    print(f"Total unique users with train interaction history: {len(user_imp)}")

    user_hist_rate = {}
    for uid, imp_u in user_imp.items():
        pos_u = user_pos.get(uid, 0)
        p_u = (pos_u + 10.0 * p_bar) / (imp_u + 10.0)
        user_hist_rate[uid] = p_u

    # 20 uniform bins on [0, 1]
    rate_edges = np.linspace(0.0, 1.0, 21)[1:-1]
    print(f"Created {len(rate_edges)} internal cutoffs for 20 user historical rate bins on [0, 1]:")
    print("  Cutoffs:", np.round(rate_edges, 4).tolist())
    ...
    def raw_features(x):
        ...
        # User historical dwell prior rate
        p_u = user_hist_rate.get(uid, p_bar)
        user_hist_rate_bin = str(int(np.searchsorted(rate_edges, p_u)))
        ...
        return [
            ...
            user_hist_rate_bin,  # user_hist_rate_bin
        ]
...
def run_experiment(k=16, lr=0.001, l2=1e-6, epochs=20, bs=8192, patience=4, seed=0):
    ...
    results = {
        'variant': 'v3',
        'hypothesis': 'User Historical Dwell / Long-View Tendency Prior Field (11 fields in DCN-FM)',
        'fields': FIELDS,
        'hyperparameters': {
            'k': k,
            'lr': lr,
