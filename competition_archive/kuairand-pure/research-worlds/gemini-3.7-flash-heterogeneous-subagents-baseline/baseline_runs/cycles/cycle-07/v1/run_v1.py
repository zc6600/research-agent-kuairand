"""Round 07 - Variant v1: Empirical Smoothed Item Popularity Prior Field (11 Fields DCN-FM)
11 Fields:
- user_id
- video_id
- author_id
...
- register_days_range
- item_pop_bin (20 uniform bins on [0, 1] for empirical smoothed item long-view prior p_v)

Empirical Smoothed Item Popularity Prior:
- Computed strictly from training log (2022-04-08 to 2022-04-21):
  p_v = (pos_v + 20 * p_bar) / (imp_v + 20)
  where p_bar = total_pos_train / total_imp_train
  For unseen items: p_v = p_bar

Model: Explicit Cross Layer + Factorization Machine (DCN-FM)
Embedding dimension: k=16, Fields=11 -> D = 11 * 16 = 176
Cross layer formulation: x_1 = x_0 * (x_0 @ W_c + b_c) + x_0, where x_0 = E_concat in R^176
...
def encode_features(splits):
    print("Encoding features across 11 fields (including 20 Log Duration Bins and 20 Item Popularity Bins)...")
    t0 = time.time()
    tr = splits['train']
    
    # 1. Calculate log-transformed duration bin edges on train split only
    ...
    edges_dur = np.linspace(min_log, max_log, 21)[1:-1]
    print(f"Created {len(edges_dur)} internal cutoffs for 20 log duration bins:")
    print("  Cutoffs:", np.round(edges_dur, 4).tolist())

    # 2. Compute empirical smoothed item long-view prior p_v strictly on train split only
    # p_v = (pos_v + 20 * p_bar) / (imp_v + 20)
    video_pos = {}
    video_imp = {}
    total_pos = 0
    ...
    for x in tr:
        vid = x[2]
        lab = x[6]
        video_imp[vid] = video_imp.get(vid, 0) + 1
        if lab == 1:
            video_pos[vid] = video_pos.get(vid, 0) + 1
            total_pos += 1
            
    p_bar = float(total_pos) / float(total_imp) if total_imp > 0 else 0.5
    print(f"Train split statistics: Total Imp={total_imp}, Total Pos={total_pos}, Global Mean p_bar={p_bar:.6f}")
    print(f"Distinct videos in train: {len(video_imp)}")

    # Precompute smoothed item prior for training videos
    vid_p_smooth = {}
    for vid, imp_v in video_imp.items():
        pos_v = video_pos.get(vid, 0)
        p_v = (pos_v + 20.0 * p_bar) / (imp_v + 20.0)
        vid_p_smooth[vid] = p_v

    # 20 uniform bins on [0, 1] for empirical item popularity prior
    edges_pop = np.linspace(0.0, 1.0, 21)[1:-1]
    print(f"Created {len(edges_pop)} internal cutoffs for 20 item popularity prior bins on [0, 1]:")
    print("  Cutoffs:", np.round(edges_pop, 4).tolist())

    def get_item_pop_bin(vid):
        p_v = vid_p_smooth.get(vid, p_bar)
        # Binned into 0..19
        return str(int(np.searchsorted(edges_pop, p_v)))

    def raw_features(x):
        # x is (date, uid, vid, author, tab, dur, label, user_active_degree, follow, fans, friend, reg_days)
        log_d = np.log1p(float(x[5]))
        dur_bucket = str(int(np.searchsorted(edges_dur, log_d)))
        pop_bin = get_item_pop_bin(x[2])
        return [
            x[1],                # user_id
            x[2],                # video_id
            x[3],                # author_id
            x[4],                # tab
            dur_bucket,          # dur_bucket (log)
            x[7],                # user_active_degree
            x[8],                # follow_user_num_range
            x[9],                # fans_user_num_range
            x[10],               # friend_user_num_range
            x[11],               # register_days_range
            pop_bin              # item_pop_bin
        ]
...
def run_experiment(k=16, lr=0.001, l2=1e-6, epochs=20, bs=8192, patience=4, seed=0):
    ...
    results = {
        'variant': 'v1',
        'hypothesis': 'Empirical Smoothed Item Popularity Prior Field (11 fields DCN-FM)',
        'fields': FIELDS,
        'hyperparameters': {
            'k': k,
            'lr': lr,
