"""Round 07 - Variant v2: Inverse Propensity Scoring (IPS) Debiasing Loss
10 Fields:
- user_id
- video_id
- author_id
...
Loss: Sample-level Inverse Propensity Loss Weights
w_i = (N / imp(v_i))^0.2, normalized so mean(w) = 1.0, clipped to [0.5, 2.0].
Down-weights over-exposed head items during BCE loss calculation.

Model: Explicit Cross Layer + Factorization Machine (DCN-FM)
Cross layer formulation: x_1 = x_0 * (x_0 @ W_c + b_c) + x_0, where x_0 = E_concat in R^160
...
class IPS_DCN_FM:
    def __init__(self, dim, num_fields=10, k=16, lr=0.001, l2=1e-6, seed=0):
    ...
    def step(self, X, y, w):
        ...
        p = sigmoid(z)
        # Weighted sample gradient: dL/dz = (w * (p - y)) / B
        g = ((w * (p - y)) / B).astype(np.float32)           # (B,)
        ...
        loss = -np.mean(w * (y * np.log(p + 1e-9) + (1 - y) * np.log(1 - p + 1e-9)))
        return float(loss)
...
def compute_ips_weights(train_rows, power=0.2, clip_min=0.5, clip_max=2.0):
    print(f"Computing Inverse Propensity Scoring (IPS) weights (power={power}, clip=[{clip_min}, {clip_max}])...")
    t0 = time.time()
    
    # Count item impressions exclusively on training partition
    item_counts = {}
    for r in train_rows:
        vid = r[2]  # video_id
        item_counts[vid] = item_counts.get(vid, 0) + 1
        
    N = len(train_rows)
    raw_weights = np.array([(N / item_counts[r[2]]) ** power for r in train_rows], dtype=np.float32)
    
    mean_raw = float(np.mean(raw_weights))
    norm_weights = raw_weights / mean_raw
    clipped_weights = np.clip(norm_weights, clip_min, clip_max).astype(np.float32)
    
    print(f"IPS Weights computed on {len(item_counts)} unique items across {N} training impressions in {time.time() - t0:.2f}s:")
    print(f"  Raw weights:     min={np.min(raw_weights):.4f}, max={np.max(raw_weights):.4f}, mean={mean_raw:.4f}")
    print(f"  Clipped weights: min={np.min(clipped_weights):.4f}, max={np.max(clipped_weights):.4f}, mean={np.mean(clipped_weights):.4f}, std={np.std(clipped_weights):.4f}")
...
def run_experiment(k=16, lr=0.001, l2=1e-6, epochs=20, bs=8192, patience=4, seed=0, ips_power=0.2, clip_min=0.5, clip_max=2.0):
    data_dir = os.path.join(PROJECT_ROOT, 'competition_data', 'data')
    splits = load_data(data_dir)
    
    # Compute IPS loss weights on train split
    w_train = compute_ips_weights(splits['train'], power=ips_power, clip_min=clip_min, clip_max=clip_max)
    ...
    print(f"\nInitializing IPS DCN_FM model (dim={dim}, fields={len(FIELDS)}, k={k}, D={len(FIELDS)*k}, lr={lr}, l2={l2}, seed={seed})...")
    m = IPS_DCN_FM(dim, num_fields=len(FIELDS), k=k, lr=lr, l2=l2, seed=seed)
    rng = np.random.default_rng(seed)
    ...
    history = []
    print(f"\nStarting IPS DCN_FM training with sample-level debiasing loss (power={ips_power}, clip=[{clip_min}, {clip_max}])...")
    ...
    for ep in range(1, epochs + 1):
        ...
        for i in range(0, len(idx), bs):
            b_idx = idx[i:i + bs]
            losses.append(m.step(Xtr[b_idx], ytr[b_idx], w_train[b_idx]))
    ...
    results = {
        'variant': 'v2',
        'hypothesis': 'Inverse Propensity Scoring (IPS) Debiasing Loss on 10-Field Log-Duration DCN-FM',
        'fields': FIELDS,
        'hyperparameters': {
            'k': k,
            'lr': lr,
