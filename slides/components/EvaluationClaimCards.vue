...
.section-kicker, .claim-label, .claim-boundary, .card-no, .card-kicker, .card-footer, .inspection-kicker { font-size: 10px; letter-spacing: 1.35px; font-weight: 850; text-transform: uppercase; }
.section-kicker { color: var(--green); }
.claim-label { margin-top: 20px; color: var(--orange); }
.claim-header h1 { margin: 9px 0 0; max-width: 720px; font-size: 34px; line-height: 1.06; letter-spacing: -1.2px; font-weight: 730; }
...
.evidence-card { position: absolute; z-index: var(--z); left: var(--x); top: var(--y); width: 148px; height: 258px; border: 0; border-radius: 20px; background: linear-gradient(180deg, rgba(255,255,255,1), rgba(250,251,252,.99)), radial-gradient(circle at 72% 18%, color-mix(in srgb, var(--accent) 9%, transparent), transparent 45%); box-shadow: 0 22px 54px rgba(27, 39, 52, .14), 0 1px 0 rgba(255,255,255,.92) inset; transform: translateY(12px) rotate(var(--rotate)) scale(var(--scale)); transform-origin: 50% 116%; color: var(--ink); text-align: left; padding: 18px 15px 15px; cursor: pointer; transition: transform .42s cubic-bezier(.22,1,.36,1), box-shadow .42s ease, opacity .35s ease, filter .35s ease; }
...
.evidence-card strong { display: block; margin-top: 28px; width: 118px; font-size: 21px; line-height: 1.04; letter-spacing: -.85px; font-weight: 790; }
