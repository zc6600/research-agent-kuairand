...
try {
  ...
  const audit = await page.evaluate(() => {
    ...
    for (const panel of document.querySelectorAll('.panel')) {
      ...
      for (const child of panel.querySelectorAll('h2,h3,p,li,td,th,.memory-row,.handoff,.terminal,.stats,.problem-note,.runtime,.parallel')) {
  ...
  pdf.setTitle('SciOdyssey · Research voyage');
  pdf.setAuthor('SciOdyssey Team');
  const output = new URL('SciOdyssey-poster-voyage.pdf', root);
  await writeFile(output, await pdf.save());
  console.log('Exported ' + fileURLToPath(output) + ' (one A0 page, 841 × 1189 mm)');
  console.log('Six sections checked; no panel overflow.');
} finally {
  await browser.close();
}
